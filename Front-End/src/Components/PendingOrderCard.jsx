...
import { updateOrderStatus } from "../API/cartRequest";
...
const PendingOrderCard = ({ order }) => {
  ...
  const { createdAt, description, image, name, price, productId, status, _id } =
    order;
    ...
    <tr className="border-b border-opacity-20 border-gray-700 bg-gray-900">
      ...
      <td className="p-3 text-right">
        <span
          ...
          className="px-3 cursor-pointer hover:bg-green-500 mr-5 py-1 font-semibold rounded-md bg-green-400 text-gray-900"
        >
          <span className="capitalize">Accept</span>
        </span>
        <span
          onClick={() => handleCancel(_id)}
          className="px-3 cursor-pointer hover:bg-red-500 py-1 font-semibold rounded-md bg-red-400 text-gray-900"
        >
